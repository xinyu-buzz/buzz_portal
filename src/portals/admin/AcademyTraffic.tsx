...
/** Friendly label for academy routes */
function labelPage(path: string): string {
  ...
  if (path.startsWith("/course/") && path.includes("/test/")) return "Exam";
  if (path.startsWith("/course/") && path.includes("/exam/")) return "Exam Intro";
  if (path.startsWith("/course/")) return "Course Details";
  if (path.startsWith("/lesson/")) return "Lesson Player";
  if (path.startsWith("/subscribe")) return "Subscribe";
  if (path.startsWith("/hanger-help/")) return "Hanger Help Post";
  if (path.startsWith("/exams/")) return "Exam Scheduling";
  return path;
}
...
export const AcademyTraffic = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (fetchError) {
        setError("Failed to load academy traffic data.");
  ...
  /* ── Page counts (grouped by label) ── */
  const pageCounts = useMemo(() => {
    const map = new Map<string, number>();
    for (const r of rows) {
      const label = labelPage(r.page_path);
          ...
          >
            ...
            <div className="chart-card">
              ...
              <div style={{ padding: 16 }}>
                {pageCounts.map(([label, count], idx) => {
            ...
            <div style={{ padding: 16 }}>
              {deviceCounts.map(([device, count], idx) => {
                ...
                return (
                  ...
                  >
                    <div style={{ width: 140, flexShrink: 0, fontSize: 13 }}>
                      {device}
                    </div>
                    <div
                      style={{
          ...
          <div className="chart-card">
            ...
            <table className="data-table">
              ...
              <tbody>
                {recentViews.map((r) => (
                  <tr key={r.id}>
                    <td title={r.page_path}>{labelPage(r.page_path)}</td>
