...
export const BookingShow = ({ basePath, role }: BookingShowProps) => {
  ...
  const toggleEditor = (editorId: string) => {
    setSelectedEditors((prev) =>
      prev.includes(editorId) ? prev.filter((p) => p !== editorId) : [...prev, editorId]
    );
  };
  ...
  const saveEditors = async () => {
    ...
    const toAdd = selectedEditors.filter((editorId) => !initialEditors.includes(editorId));
    const toRemove = initialEditors.filter((editorId) => !selectedEditors.includes(editorId));
      ...
      try {
        ...
        data.forEach((place: any) => {
          if (place.lat && place.lon) {
            items.push({
              title:
                place.name || place.display_name?.split(",")[0] || editForm.address,
  ...
  return (
    <div className="page-card">
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
            {selectedEditors.length ? (
              selectedEditors.map((editorId) => {
                const label =
                  editorOptions.find((opt) => opt.id === editorId)?.label || "Editor";
                return (
                  <div
                    key={editorId}
                    style={{
      ...
      <BookingMediaManager
        bookingId={bookingId}
        uploaderRole={role === "editor" ? "editor" : role === "pilot" ? "pilot" : "system"}
      />
        ...
        <div className="modal-backdrop">
          <div className="modal-card">
            ...
            <form className="modal-form" onSubmit={handleUpdate}>
              ...
              <input type="hidden" name="location_lat" value={editForm.location_lat} />
              <input type="hidden" name="location_lng" value={editForm.location_lng} />
